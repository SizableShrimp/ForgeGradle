...
public class MinecraftUserRepo extends BaseRepo {
    ...
    private static final MinecraftVersion v1_17 = MinecraftVersion.from("1.17");
    ...
    private File findPom(@Nullable String mapping, String rand) throws IOException {
        ...
        if (cache.isSame() && pom.exists()) {
            ...
            if (this.mcp == null && "net.minecraft".equals(GROUP) && "client".equals(NAME)) {
                // Use alias cuz gradle classifier dumbness
    ...
    private File findDecomp(boolean generate) throws IOException {
        ...
        if (shouldGenerateDecomp && this.mcp != null) {
            ...
            decompileJar.getArgs().addAll("-din=1", "-rbr=1", "-dgs=1", "-asc=1", "-rsy=1", "-iec=1", "-jvn=1", "-isl=0", "-iib=1", "-bsm=1", "-dcl=1");
            raw = findRaw(MAPPING);
            if (raw == null)
                return null;
